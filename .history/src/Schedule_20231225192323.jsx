...
const Schedule = () => {
  ...
  const CustomTooltip = ({ appointmentData }) => {
    const { title, startDate, endDate, color } = appointmentData;
  
    return (
      <div style={{ display: 'flex', flexDirection: 'column', padding: 10 }}>
        <div style={{ fontWeight: 'bold' }}>{title}</div>
        <div>Time: {startDate} - {endDate}</div>
        <div style={{ backgroundColor: color, width: 10, height: 10, margin: 5 }} />
      </div>
    );
  };
    ...
    <div className=" flex items-center justify-center my-6">
      ...
      <Paper style={{ width: "90%", height: 550 }}>
        <Scheduler data={state}>
          ...
          />
          <AppointmentTooltip showCloseButton showOpenButton contentComponent={CustomTooltip} />
          <AppointmentForm />
        </Scheduler>
      </Paper>
    </div>
