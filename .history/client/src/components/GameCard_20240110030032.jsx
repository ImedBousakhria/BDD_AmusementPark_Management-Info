...
const GameCard = () => {
  return (
    <div className="flex flex-col bg-white h-max w-max rounded-md p-2">
      ...
      <div className="flex">
        <img src={game} alt="" />
        <Details title={"Thriller Seeker Express"} genre={"Attraction"} age={16} currentStatus={"Open"}/>
      </div>
      <small className=" text-customPink text-gree font-medium cursor-pointer place-self-end">
        See in map
      </small>
    </div>
  );
...
const Details = ({ title, userType, genre, age, currentStatus }) => {
  ...
  switch (currentStatus) {
    case 'open':
      ...
      break;
    case 'closed':
      statusColorClass = 'red-text';
      break;
    case 'under maintenance':
      statusColorClass = 'orange-text';
      break;
    default:
      statusColorClass = '';
  }
