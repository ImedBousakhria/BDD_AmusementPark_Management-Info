...
router.get("/feedback", (req, res) => {
  connection.query("SELECT * FROM feedback", (error, results) => {
    if (error) {
      ...
      return res
        .status(500)
        .json({ message: "There was an error getting the data" });
    }
    res.json(results);
  });
});
    ...
    connection.query(
      ...
      async (error, results) => {
        ...
        try {
          ...
          console.log(
            "An error occurred while trying to hash the password",
            error
          );
          return res.status(500).send("Internal Server Error");
        }
...
try {
  ...
  // Compare the provided password with the hashed password from the database
  const passwordMatch = await middleware.comparePasswords(
    password,
    user.password
  );
