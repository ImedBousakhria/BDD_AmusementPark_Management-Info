          ...
          (err, rows) => {
            ...
            if (!err) {
              res.render("edit-staff", {
                rows,
                alert: `${first_name} staff updated successfully!`,
              });
            } else {
              console.log(err);
            }
...
exports.editZone = (req, res) => {
  // Use the connection
  connectionPool.query(
    "SELECT * FROM zone WHERE id_zone = ?",
    [req.params.id],
    (err, rows) => {
      if (!err) {
      ...
      if (!err) {
        ...
        connection.query(
          ...
          (err, rows) => {
            ...
            if (!err) {
              res.render("edit-zone", {
                rows,
                alert: "${first_name} User updated successfully!",
              });
            } else {
              console.log(err);
            }
...
exports.deleteZone = (req, res) => {
  // user connection
  connection.query(
    "DELETE FROM zone WHERE id_zone = ?",
    [req.params.id],
    (err, result) => {
      if (!err) {
