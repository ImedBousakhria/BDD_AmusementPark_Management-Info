...
const Schedule = () => {
  ...
  // so important : basic is for the default form, command is for well command, reccurence is only a thing once isReccurent = True
  const FormLayout = (props) => {
    const { basicLayoutComponent } = props;
    ...
    return (
      ...
      >
        ...
        <div onClick={() => console.log(props)}>EY Shawty click to unveil my props</div>
      </AppointmentForm.Layout>
    );
  };
 ...
 /*  const FormOverlay = (props) => {
    ...
    return (
      ...
      >
        {children}
        <div onClick={() => console.log(children)}>Click to see children and target</div>
      </AppointmentForm.Overlay>
    );
  }; */
    ...
    <div className=" flex items-center justify-center py-6 h-screen bg-darkBG">
      ...
      <Paper style={{ width: "90%", height: 600 }}>
        <Scheduler data={state}>
          ...
          <AppointmentForm layoutComponent={FormLayout} overlayComponent={FormOverlay} />
        </Scheduler>
      </Paper>
    </div>
  );
