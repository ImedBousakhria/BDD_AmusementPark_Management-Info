  ...
  return (
    ...
    >
      ...
      <ResponsiveBar
        ...
        data={allData}
        keys={["value"]}
        indexBy="rank"/* {(d) => `${d.rank} - ${d.name}`} / */ // Combine Rank and name for indexing
        ...
        layout="horizontal"
        label={({ index }) => (
          <g transform={`translate(${index * 45}, 20)`}>
            <text x={0} y={0} textAnchor="middle" dy={16} fontSize={12} fill="#333">
              {allData[index].rank}
            </text>
            <text x={0} y={20} textAnchor="middle" dy={16} fontSize={12} fill="#333">
              {allData[index].name}
            </text>
          </g>
        )}
